...
export default async function AdminDisputeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch dispute record (may not exist for disputes opened before this feature)
  ...
  return (
    <main className="page" style={{ background: "var(--background)", minHeight: "100vh" }}>
      <div className="admin-content" style={{ maxWidth: 780 }}>
        ...
        {/* Dispute thread (evidence + messages) */}
        {dispute && (
          <div style={{ marginBottom: "1rem" }}>
            <DisputeThread
              disputeId={dispute.id}
              messages={dispute.messages.map((m) => ({
                id: m.id, body: m.body, isSystem: m.isSystem,
