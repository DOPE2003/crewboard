    ...
    <main className="page" style={{ background: "var(--background)", minHeight: "100vh" }}>
      <div className="admin-content">
        ...
        <div style={{ marginBottom: "2.5rem" }}>
          <div style={{ fontSize: "0.6rem", letterSpacing: "0.2em", color: "#ef4444", marginBottom: "0.5rem", fontWeight: 700 }}>
            — COMMAND CENTER
          </div>
          <h1 style={{ fontSize: "2rem", fontWeight: 800, color: "var(--foreground)", margin: 0 }}>Admin Dashboard</h1>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1rem", marginBottom: "2.5rem" }}>
          {[
            ...
            { label: "Total Revenue",    value: `$${revenue.toLocaleString()}`, color: "#ec4899", raw: true },
            ...
            <Link key={s.href} href={s.href} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                ...
                <span style={{ fontSize: "1.25rem", fontWeight: 800, color: s.color, fontFamily: "Space Mono, monospace" }}>
                  {s.count}{s.countLabel ? <span style={{ fontSize: "0.6rem", fontWeight: 600, marginLeft: 3, color: "var(--text-muted)" }}>{s.countLabel}</span> : ""}
                </span>
              </div>
        ...
        <div style={{ background: "var(--card-bg)", borderRadius: 16, border: "1px solid var(--card-border)", overflow: "hidden" }}>
          <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid var(--card-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <Link href="/admin/users" style={{ fontSize: "0.75rem", color: "#14b8a6", textDecoration: "none", fontWeight: 600 }}>
              View all →
            </Link>
          </div>
                  ...
                  <tr key={user.id} style={{ borderTop: "1px solid var(--card-border)" }}>
                    ...
                    <td style={{ padding: "0.9rem 1.5rem" }}>
                      <span style={{
                        ...
                        background: user.role === "ADMIN" ? "#fef2f2" : "rgba(var(--foreground-rgb),0.05)",
