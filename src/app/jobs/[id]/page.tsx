    ...
    <main className="page" style={{ minHeight: "100vh" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "2.5rem 1.5rem 6rem" }}>
        ...
        <div style={{ marginTop: 24, display: "flex", gap: 12, flexWrap: "wrap" }}>
          {isOwner ? (
            <>
              <Link href={`/jobs/${id}/applicants`} style={{ padding: "12px 24px", borderRadius: 10, background: "var(--brand)", color: "#0f172a", fontWeight: 700, fontSize: 15, textDecoration: "none" }}>
                Manage Applicants ({job._count.applications})
              </Link>
              <Link href={`/jobs/${id}/edit`} style={{ padding: "12px 24px", borderRadius: 10, border: "1px solid var(--border)", color: "var(--foreground)", fontWeight: 600, fontSize: 15, textDecoration: "none" }}>
                Edit Job
              </Link>
              <Link href="/jobs/my" style={{ padding: "12px 24px", borderRadius: 10, border: "1px solid var(--border)", color: "var(--foreground)", fontWeight: 600, fontSize: 15, textDecoration: "none" }}>
                My Jobs
              </Link>
            </>
          ) : isLoggedIn ? (
