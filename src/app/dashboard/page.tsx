...
export default async function DashboardPage() {
  ...
  const [dbUser, notifications, recentConvos] = await Promise.all([
    db.user.findUnique({
      where: { id: userId },
  ...
  // Fetch the other participant for each conversation (regardless of who sent the last message)
    ...
    <main style={{ minHeight: "100vh", paddingTop: "8.5rem", paddingLeft: "1.5rem", paddingRight: "1.5rem", paddingBottom: "5rem" }}>
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1.25rem" }} className="dash-two-col">
          <div style={{ borderRadius: 14, padding: "1.1rem 1.25rem", background: "#fff", border: "1px solid rgba(0,0,0,0.07)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.9rem" }}>
              <span style={{ fontFamily: "Space Mono,monospace", fontSize: "0.58rem", letterSpacing: "0.14em", textTransform: "uppercase", color: "#94a3b8" }}>Your Projects</span>
              <Link href="/projects" style={{ fontSize: "0.65rem", color: "#2DD4BF", textDecoration: "none", fontWeight: 600 }}>View all</Link>
            </div>
            <div style={{ fontSize: "0.75rem", color: "#cbd5e1" }}>No active projects yet.</div>
          </div>

          <div style={{ borderRadius: 14, padding: "1.1rem 1.25rem", background: "#fff", border: "1px solid rgba(0,0,0,0.07)" }}>
            <div style={{ fontFamily: "Space Mono,monospace", fontSize: "0.58rem", letterSpacing: "0.14em", textTransform: "uppercase", color: "#94a3b8", marginBottom: "0.9rem" }}>Quick Actions</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {[
                ...
                <Link key={a.href} href={a.href} style={{
                  ...
                  fontWeight: 500, color: "#0f172a", textDecoration: "none",
                  background: "#f8fafc", border: "1px solid rgba(0,0,0,0.05)",
                }}>
                  {a.label} <span style={{ color: "#94a3b8" }}>→</span>
                </Link>
              ))}
            </div>
          </div>
