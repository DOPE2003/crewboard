  ...
  return (
    <main className="dash-main-wrap mf-page" style={{ minHeight: "100vh", paddingBottom: "5rem" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: "0.7rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase" as const, color: "#14b8a6" }}>
                  Active Work — {activeOrders.length} order{activeOrders.length !== 1 ? "s" : ""}
                </span>
              </div>
              <Link href="/orders" style={{ fontSize: "0.75rem", fontWeight: 600, color: "#14b8a6", textDecoration: "none" }}>
                View all →
              </Link>
            </div>
                ...
                return (
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: "0.72rem", color: statusColor[o.status] ?? "var(--text-muted)", fontWeight: 600, marginTop: 2 }}>
                        {statusLabel[o.status] ?? o.status}
                      </div>
                    </div>
                    <div style={{ flexShrink: 0, fontSize: "0.82rem", fontWeight: 700, color: "var(--foreground)" }}>
                      ${o.amount}
                    </div>
        ...
        {/* ── 6. POSTED JOBS ── */}
        {postedJobs.length > 0 && (
          <PostedJobsSection jobs={postedJobs} />
        )}

        {/* ── 7. SENT OFFERS ── */}
        {sentOffers.length > 0 && (
          <SentOffersSection offers={sentOffers as any} />
        )}

        {/* ── 8. WALLET — compact, low priority ── */}
        <div style={{
